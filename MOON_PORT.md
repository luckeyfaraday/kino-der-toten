...
Rebuilding needs Python 3.11/Pillow, FFmpeg, Node and retained local OAT dumps. Shared Kino exports must exist first. To refresh raw extraction, run `.tools/rebuild-moon.ps1 -Extract`; this additionally uses `.tools/Unlinker.exe` and the patched WSL exporter `/home/alanq/oat/build/bin/Release_x64/Unlinker` (override with `-PatchedUnlinker`). Its patch is retained under `.tools/oat_patch`.
